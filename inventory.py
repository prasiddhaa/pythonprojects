...
def show():
    ...
    if (var2.get()):
        ...
        qty = int(e7.get())
        tot = int(price * qty)
        tempList = [['Vodka', e2.get(), e7.get(), tot]]
        tempList.sort(key=lambda e: e[1], reverse=True)
        for i, (item, price, qty, tot) in enumerate(tempList, start=1):
            listBox.insert("", "end", values=(item, price, qty, tot))
    ...
    if (var3.get()):
        ...
        qty = int(e8.get())
        tot = int(price * qty)
        tempList = [['Whisky', e3.get(), e8.get(), tot]]
        tempList.sort(key=lambda e: e[1], reverse=True)
        for i, (item, price, qty, tot) in enumerate(tempList, start=1):
            listBox.insert("", "end", values=(item, price, qty, tot))
    ...
    if (var4.get()):
        ...
        qty = int(e9.get())
        tot = int(price * qty)
        tempList = [['Juice', e4.get(), e9.get(), tot]]
        tempList.sort(key=lambda e: e[1], reverse=True)
    ...
    if (var5.get()):
        ...
        qty = int(e10.get())
        tot = int(price * qty)
        tempList = [['Beer', e5.get(), e10.get(), tot]]
        tempList.sort(key=lambda e: e[1], reverse=True)
        for i, (item, price, qty, tot) in enumerate(tempList, start=1):
            listBox.insert("", "end", values=(item, price, qty, tot))
...
root = Tk()
root.title("BILL PRINT INVENTORY SYSTEM")
...
Label(root, text="BILL PRINT INVENTORY SYSTEM", font="arial 22 bold" ,bg="white").place(x=5, y=10)
...
var2 = IntVar()
Checkbutton(root, text="Vodka", variable=var2).place(x=10, y=80)

var3 = IntVar()
Checkbutton(root, text="Whisky", variable=var3).place(x=10, y=110)

var4 = IntVar()
Checkbutton(root, text="Juice", variable=var4).place(x=10, y=140)

var5 = IntVar()
Checkbutton(root, text=" Beer  ", variable=var5).place(x=10, y=170)
...
Button(root, text="Add", command=show, height=1, width=13).place(x=10, y=220)

Button(root, text="Print", command=print, height=1, width=13).place(x=850, y=120)
